...
const workTypeOptions = [
  { value: "book", label: "წიგნი", description: "Long-form work with chapter outline", icon: BookOpen },
  { value: "poem", label: "Poem", description: "Poetry draft with rich formatting", icon: Feather },
  { value: "story", label: "მოთხრობა", description: "Short story writing workspace", icon: FileText },
  { value: "chapter", label: "თავი", description: "Create chapter under an existing book", icon: PlusSquare },
] as const;
...
const WriterNewWorkPage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      if (workType === "book") {
        if (sourceType === "upload" && !uploadFile) {
          throw new Error("ატვირთვის რეჟიმისთვის აირჩიე ფაილი");
        }
      ...
      if (workType === "poem") {
        ...
        const created = await createPoem({
          ...
          description: "",
          body: sourceType === "manual" ? "<p>დაიწყე ლექსის წერა...</p>" : "",
          source_type: sourceType,
          upload_file: sourceType === "upload" ? uploadFile : null,
          is_anonymous: isAnonymous,
        });
      ...
      if (workType === "story") {
        ...
        const created = await createStory({
          ...
          description: "",
          body: sourceType === "manual" ? "<p>დაიწყე მოთხრობის წერა...</p>" : "",
          source_type: sourceType,
          upload_file: sourceType === "upload" ? uploadFile : null,
          is_anonymous: isAnonymous,
        });
      ...
      if (!bookIdForChapter) {
        throw new Error("ჯერ აირჩიე წიგნი");
      }
    ...
    onSuccess: ({ route }) => {
      toast({
        title: "დრაფტი შეიქმნა",
        description: "რედაქტორი იხსნება...",
      });
      navigate(route);
    },
    onError: (error) => {
    ...
    <div className="container mx-auto max-w-3xl space-y-6 px-6 py-10">
      <section className="rounded-2xl border border-border/70 bg-card/80 p-8 shadow-card">
        <div className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/70 px-3 py-1">
          ...
          <span className="font-ui text-xs text-muted-foreground">ავტორის სამუშაო სივრცე</span>
        </div>

        <h1 className="mt-4 font-display text-4xl font-semibold text-foreground">ახალი ნაშრომის შექმნა</h1>
        ...
        <div className="mt-6 space-y-5">
          <div className="space-y-2">
            <Label className="font-ui">კონტენტის ტიპი</Label>
            <Select
              value={workType}
              onValueChange={(value) => {
          ...
          {supportsUploadChoice ? (
            <div className="flex items-start justify-between rounded-xl border border-border/70 bg-background/70 p-4">
              <div className="space-y-1 pr-4">
                <p className="font-ui text-sm font-medium text-foreground">გამოქვეყნება ანონიმურად</p>
                <p className="font-ui text-xs text-muted-foreground">
                  საჯარო მკითხველები და რედაქტორები ნახავენ მხოლოდ "ანონიმურს". შენი ვინაობა მხოლოდ ადმინებს ექნებათ ხილული.
                </p>
              </div>
          ...
          {supportsUploadChoice && sourceType === "upload" ? (
            <div className="space-y-2">
              <Label htmlFor="newWorkUpload" className="font-ui">ფაილის ატვირთვა</Label>
              <Input
                id="newWorkUpload"
                type="file"
              ...
              />
              <p className="font-ui text-xs text-muted-foreground">დაშვებულია: PDF, DOC, DOCX, TXT (მაქს. 20MB).</p>
            </div>
          ) : null}
          ...
          {workType === "chapter" ? (
            <div className="space-y-2">
              <Label className="font-ui">მშობელი წიგნის არჩევა</Label>
