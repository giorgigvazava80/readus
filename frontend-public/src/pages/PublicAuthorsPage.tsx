      ...
      <div className="container mx-auto px-4 sm:px-6 py-8 md:py-10">
        {authorsQuery.isLoading ? (
          <div className="grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="rounded-2xl border bg-card shadow-card overflow-hidden animate-pulse p-5 space-y-4">
              ...
              {authors.map((author) => {
                ...
                return (
                  <Link key={author.key} to={authorProfilePath(author.key)} className="block h-full">
                    <article className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
                      <div className="flex items-center gap-3">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full border border-border/60 bg-muted/40">
                          {profilePhoto ? (
                            <img src={profilePhoto} alt={author.display_name} className="h-full w-full object-cover" />
                          ) : (
                            <span className="font-ui text-sm font-semibold text-muted-foreground">{fallback}</span>
                          )}
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-lg border border-border/50 bg-muted/20 px-2 py-2">
                          <BookOpen className="mx-auto h-3.5 w-3.5 text-muted-foreground" />
                          <p className="mt-1 font-ui text-xs text-muted-foreground">{author.books_count}</p>
                        </div>
                        <div className="rounded-lg border border-border/50 bg-muted/20 px-2 py-2">
                          <FileText className="mx-auto h-3.5 w-3.5 text-muted-foreground" />
                          <p className="mt-1 font-ui text-xs text-muted-foreground">{author.stories_count}</p>
                        </div>
                        <div className="rounded-lg border border-border/50 bg-muted/20 px-2 py-2">
                          <PenLine className="mx-auto h-3.5 w-3.5 text-muted-foreground" />
                          <p className="mt-1 font-ui text-xs text-muted-foreground">{author.poems_count}</p>
                        </div>
                      </div>
